...
class GA:
    def __init__(self, cities, num_genes=90, num_children=90):
        ...
        def Iter(self, max_iter):
            ...
            bestLength, bestPath = bestgene.Length, bestgene.Path
            ...
            print(f'Iterate times: {bestItetate}, Length: {bestLength}, Path: {bestPath}')
            self._Path(bestItetate, bestLength, bestPath)
    ...
    def _Path(self, Itertime, Length, Path):
        for i in range(-1, self.num_cities - 1):
            ...
            plt.plot(x, y)
        plt.title(f'Current Iterate times: {Itertime}, Length: {Length:.6f}')
        plt.show()
